    ...
    try {
      ...
      for (const listino of allListiniSet) {
        try {
          ...
          if (!parentPendente) {
            const [rows] = await dbL.query(`SELECT ${COLS} FROM listini WHERE categoria = ? AND disponibile = 1 AND preventivabile = 1 AND principale = 1 ORDER BY descrizione ASC`, [listino])
            articoli = rows as ArticoloListino[]
          } else if (lacuneAperte.length === 0) {
            const [rows] = await dbL.query(`SELECT ${COLS} FROM listini WHERE categoria = ? AND disponibile = 1 AND preventivabile = 1 AND caratteristica = 1 ORDER BY descrizione ASC`, [listino])
            articoli = rows as ArticoloListino[]
          } else {
            const [rows] = await dbL.query(`SELECT ${COLS} FROM listini WHERE categoria = ? AND disponibile = 1 AND preventivabile = 1 AND caratteristica = 1 ORDER BY descrizione ASC`, [listino])
  ...
  return (
    <div className="fs-15" style={{ padding: '0 0 80px', color: '#444', lineHeight: 1.8 }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 8 }}>
        <Link href="/app/cataloghi" className="btn-black fs-12" style={{ flex: 1, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', height: 42, padding: '0 20px', borderRadius: 21, textDecoration: 'none', fontFamily: 'monospace' }}>
          ← Torna ai Cataloghi
        </Link>
        {cartNonVuoto && (
          <Link href="/app/carrello-preventivo" className="btn-black fs-12" style={{ flex: 1, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', height: 42, padding: '0 20px', borderRadius: 21, textDecoration: 'none', fontFamily: 'monospace' }}>
