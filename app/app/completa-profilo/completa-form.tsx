...
export default function CompletaForm({ user }: { user: UserData }) {
  const router = useRouter()
  const [phase, setPhase] = useState<Phase>('form')
  const [pendingId, setPendingId] = useState<number | null>(null)
  const [needsSmsAfterEmail, setNeedsSmsAfterEmail] = useState(false)

  // Cellulare: mostra senza +39 per la parte editabile
    ...
    async (prev, fd) => {
      ...
      if (res.ok) {
        if (res.saved) {
          ...
          setNeedsSmsAfterEmail(res.needsSms)
