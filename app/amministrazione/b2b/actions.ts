...
export async function salvaTemplateB2B(_: MutResult | null, fd: FormData): Promise<MutResult> {
  await checkAdmin()
  ...
  try {
    if (id) {
      await db.execute('UPDATE b2b_templates SET oggetto = ?, testo = ? WHERE id = ?', [oggetto, testo, id])
      revalidatePath('/amministrazione/b2b')
      return { ok: true, id }
    }
    const [res] = await db.execute('INSERT INTO b2b_templates (oggetto, testo) VALUES (?, ?)', [oggetto, testo])
    revalidatePath('/amministrazione/b2b')
...
export async function salvaComeNuovoTemplateB2B(_: MutResult | null, fd: FormData): Promise<MutResult> {
  await checkAdmin()
  const oggetto = ((fd.get('oggetto') as string) ?? '').trim()
  const testo   = ((fd.get('testo') as string) ?? '').trim()
...
export async function eliminaTemplateB2B(_: MutResult | null, fd: FormData): Promise<MutResult> {
  await checkAdmin()
  const id = parseInt(fd.get('id') as string)
  if (!id) return { ok: false, error: 'ID non valido.' }
  const db = await getConnection()
  try {
    await db.execute('DELETE FROM b2b_templates WHERE id = ?', [id])
    revalidatePath('/amministrazione/b2b')
    return { ok: true }
  } finally {
    await db.end()
...
export async function salvaBrandB2B(_: MutResult | null, fd: FormData): Promise<MutResult> {
  await checkAdmin()
  const id       = parseInt(fd.get('id') as string) || null
  ...
  try {
    if (id) {
      await db.execute('UPDATE b2b_brand SET nome = ?, email = ?, telefono = ?, note = ? WHERE id = ?', [nome, email, telefono, note, id])
    } else {
      await db.execute('INSERT INTO b2b_brand (nome, email, telefono, note) VALUES (?, ?, ?, ?)', [nome, email, telefono, note])
    }
    revalidatePath('/amministrazione/b2b')
    return { ok: true }
  } finally {
...
export async function eliminaBrandB2B(_: MutResult | null, fd: FormData): Promise<MutResult> {
  await checkAdmin()
  const id = parseInt(fd.get('id') as string)
  if (!id) return { ok: false, error: 'ID non valido.' }
  const db = await getConnection()
  try {
    await db.execute('DELETE FROM b2b_brand WHERE id = ?', [id])
    revalidatePath('/amministrazione/b2b')
    return { ok: true }
  } finally {
    await db.end()
...
export async function inviaEmailB2B(oggetto: string, testo: string, brandIds: number[]): Promise<InviaResult> {
  await checkAdmin()
  ...
  if (brandIds.length === 0) return { ok: false, error: 'Seleziona almeno un destinatario.' }
  ...
  try {
    ...
    const [rows] = await db.query(
      `SELECT id, email FROM b2b_brand WHERE id IN (${placeholders})`,
      brandIds
    )
