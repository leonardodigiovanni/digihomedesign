'use server'

import { getConnection } from '@/lib/db'
import { revalidatePath } from 'next/cache'

async function ensureTables() {
  const db = await getConnection()

  await db.execute(`
    CREATE TABLE IF NOT EXISTS cantieri (
      id               INT AUTO_INCREMENT PRIMARY KEY,
      cliente_id       INT NULL,
      titolo           VARCHAR(200) NOT NULL,
      indirizzo        VARCHAR(300) NOT NULL DEFAULT '',
      stato            ENUM('preventivo','in_corso','completato','sospeso') NOT NULL DEFAULT 'preventivo',
      data_preventivo  DATE NULL,
      inizio_lavori    DATE NULL,
      fine_lavori      DATE NULL,
      note_pubbliche   TEXT NULL,
      note_interne     TEXT NULL,
      visibile_cliente TINYINT(1) NOT NULL DEFAULT 1,
      created_at       TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    )
  `)

  await db.execute(`
    CREATE TABLE IF NOT EXISTS cantieri_lavori (
      id          INT AUTO_INCREMENT PRIMARY KEY,
      cantiere_id INT NOT NULL,
      descrizione VARCHAR(300) NOT NULL,
      data_inizio DATE NULL,
      data_fine   DATE NULL,
      note        TEXT NULL,
      created_at  TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
      FOREIGN KEY (cantiere_id) REFERENCES cantieri(id) ON DELETE CASCADE
    )
  `)

  await db.execute(`
    CREATE TABLE IF NOT EXISTS cantieri_media (
      id          INT AUTO_INCREMENT PRIMARY KEY,
      task_id     INT NOT NULL,
      tipo        ENUM('foto','video') NOT NULL DEFAULT 'foto',
      filename    VARCHAR(255) NOT NULL,
      descrizione VARCHAR(200) NULL,
      visto       TINYINT(1) NOT NULL DEFAULT 0,
      created_at  TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
      FOREIGN KEY (task_id) REFERENCES cantieri_lavori(id) ON DELETE CASCADE
    )
  `)

  // Migrations
  try { await db.execute('ALTER TABLE cantieri ADD COLUMN data_preventivo DATE NULL') } catch { /* esiste già */ }
  try { await db.execute('ALTER TABLE cantieri ADD COLUMN visibile_cliente TINYINT(1) NOT NULL DEFAULT 1') } catch { /* esiste già */ }
  try { await db.execute('ALTER TABLE cantieri_lavori ADD COLUMN data_inizio DATE NULL') } catch { /* esiste già */ }
  try { await db.execute('ALTER TABLE cantieri_lavori ADD COLUMN data_fine DATE NULL') } catch { /* esiste già */ }
  try { await db.execute('ALTER TABLE cantieri_lavori ADD COLUMN note TEXT NULL') } catch { /* esiste già */ }
  try { await db.execute("ALTER TABLE cantieri_lavori ADD COLUMN stato ENUM('da_fare','in_corso','completato','sospeso') NOT NULL DEFAULT 'da_fare'") } catch { /* esiste già */ }
  try { await db.execute('ALTER TABLE cantieri_lavori ADD COLUMN created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP') } catch { /* esiste già */ }
  try { await db.execute('ALTER TABLE cantieri_media ADD COLUMN task_id INT NULL') } catch { /* esiste già */ }
  try { await db.execute('ALTER TABLE cantieri_media ADD COLUMN visto TINYINT(1) NOT NULL DEFAULT 0') } catch { /* esiste già */ }
  try { await db.execute('ALTER TABLE cantieri_media MODIFY COLUMN cantiere_id INT NULL DEFAULT NULL') } catch { /* esiste già o no FK */ }

  await db.end()
}

function dateField(fd: FormData, key: string): string | null {
  const v = (fd.get(key) as string ?? '').slice(0, 10)
  return v ? v + 'T12:00:00' : null
}

// ─── Cantieri ─────────────────────────────────────────────────────────────────

export async function addCantiere(_: unknown, formData: FormData) {
  await ensureTables()

  const cliente_id = parseInt(formData.get('cliente_id') as string) || null
  const titolo     = (formData.get('titolo')    as string) ?? ''
  const indirizzo  = (formData.get('indirizzo') as string) ?? ''
  const stato      = (formData.get('stato')     as string) ?? 'preventivo'
  const prev       = dateField(formData, 'data_preventivo')
  const inizio     = dateField(formData, 'inizio_lavori')
  const fine       = dateField(formData, 'fine_lavori')
  const note_pub   = (formData.get('note_pubbliche') as string) ?? ''
  const note_int   = (formData.get('note_interne')   as string) ?? ''

  if (!titolo) return { error: 'Il titolo è obbligatorio.' }

  const db = await getConnection()
  await db.execute(
    'INSERT INTO cantieri (cliente_id, titolo, indirizzo, stato, data_preventivo, inizio_lavori, fine_lavori, note_pubbliche, note_interne) VALUES (?,?,?,?,?,?,?,?,?)',
    [cliente_id, titolo, indirizzo, stato, prev, inizio, fine, note_pub, note_int]
  )
  await db.end()
  revalidatePath('/area-lavoro/cantieri')
  return { success: true }
}

export async function assignClienteToCantiere(_: unknown, formData: FormData) {
  const id         = formData.get('id') as string
  const cliente_id = parseInt(formData.get('cliente_id') as string) || null
  if (!id) return { error: 'ID mancante.' }
  await ensureTables()
  const db = await getConnection()
  await db.execute('UPDATE cantieri SET cliente_id = ? WHERE id = ?', [cliente_id, id])
  await db.end()
  revalidatePath('/area-lavoro/cantieri')
  return { success: true }
}

export async function updateStatoCantiere(_: unknown, formData: FormData) {
  const id    = formData.get('id')    as string
  const stato = formData.get('stato') as string
  if (!id || !stato) return { error: 'Dati mancanti.' }
  await ensureTables()
  const db = await getConnection()
  await db.execute('UPDATE cantieri SET stato = ? WHERE id = ?', [stato, id])
  await db.end()
  revalidatePath('/area-lavoro/cantieri')
  return { success: true }
}

export async function toggleVisibileCantiere(_: unknown, formData: FormData) {
  const id = formData.get('id') as string
  if (!id) return { error: 'ID mancante.' }
  await ensureTables()
  const db = await getConnection()
  await db.execute('UPDATE cantieri SET visibile_cliente = 1 - visibile_cliente WHERE id = ?', [id])
  await db.end()
  revalidatePath('/area-lavoro/cantieri')
  return { success: true }
}

export async function deleteCantiere(_: unknown, formData: FormData) {
  const id = formData.get('id') as string
  if (!id) return { error: 'ID mancante.' }
  await ensureTables()
  const db = await getConnection()
  await db.execute('DELETE FROM cantieri WHERE id = ?', [id])
  await db.end()
  revalidatePath('/area-lavoro/cantieri')
  return { success: true }
}

// ─── Task ─────────────────────────────────────────────────────────────────────

export async function addTask(_: unknown, formData: FormData) {
  await ensureTables()

  const cantiere_id = parseInt(formData.get('cantiere_id') as string)
  const descrizione = (formData.get('descrizione') as string) ?? ''
  const data_inizio = dateField(formData, 'data_inizio')
  const data_fine   = dateField(formData, 'data_fine')
  const note        = (formData.get('note') as string) ?? ''
  const stato       = (formData.get('stato') as string) || 'da_fare'

  if (!cantiere_id || !descrizione) return { error: 'Dati mancanti.' }

  const db = await getConnection()
  await db.execute(
    'INSERT INTO cantieri_lavori (cantiere_id, descrizione, data_inizio, data_fine, note, stato) VALUES (?,?,?,?,?,?)',
    [cantiere_id, descrizione, data_inizio, data_fine, note || null, stato]
  )
  await db.end()
  revalidatePath('/area-lavoro/cantieri')
  return { success: true }
}

export async function updateStatoTask(_: unknown, formData: FormData) {
  const id    = formData.get('id')    as string
  const stato = formData.get('stato') as string
  if (!id || !stato) return { error: 'Dati mancanti.' }
  const db = await getConnection()
  await db.execute('UPDATE cantieri_lavori SET stato = ? WHERE id = ?', [stato, id])
  await db.end()
  revalidatePath('/area-lavoro/cantieri')
  return { success: true }
}

export async function updateTask(_: unknown, formData: FormData) {
  const id          = formData.get('id') as string
  const descrizione = (formData.get('descrizione') as string) ?? ''
  const data_inizio = dateField(formData, 'data_inizio')
  const data_fine   = dateField(formData, 'data_fine')
  const stato       = (formData.get('stato') as string) || 'da_fare'
  if (!id || !descrizione) return { error: 'Dati mancanti.' }
  const db = await getConnection()
  await db.execute(
    'UPDATE cantieri_lavori SET descrizione = ?, data_inizio = ?, data_fine = ?, stato = ? WHERE id = ?',
    [descrizione, data_inizio, data_fine, stato, id]
  )
  await db.end()
  revalidatePath('/area-lavoro/cantieri')
  return { success: true }
}

export async function deleteTask(_: unknown, formData: FormData) {
  const id = formData.get('id') as string
  if (!id) return { error: 'ID mancante.' }
  await ensureTables()
  const db = await getConnection()
  await db.execute('DELETE FROM cantieri_lavori WHERE id = ?', [id])
  await db.end()
  revalidatePath('/area-lavoro/cantieri')
  return { success: true }
}

// ─── Media ────────────────────────────────────────────────────────────────────

export async function addMedia(_: unknown, formData: FormData) {
  await ensureTables()

  const task_id     = parseInt(formData.get('task_id') as string)
  const filename    = (formData.get('filename')    as string) ?? ''
  const tipo        = (formData.get('tipo')        as string) ?? 'foto'
  const descrizione = (formData.get('descrizione') as string) ?? ''

  if (!task_id || !filename) return { error: 'Dati mancanti.' }

  const db = await getConnection()
  await db.execute(
    'INSERT INTO cantieri_media (task_id, tipo, filename, descrizione) VALUES (?,?,?,?)',
    [task_id, tipo, filename, descrizione || null]
  )
  await db.end()
  revalidatePath('/area-lavoro/cantieri')
  return { success: true }
}

export async function deleteMedia(_: unknown, formData: FormData) {
  const id = formData.get('id') as string
  if (!id) return { error: 'ID mancante.' }
  await ensureTables()
  const db = await getConnection()
  await db.execute('DELETE FROM cantieri_media WHERE id = ?', [id])
  await db.end()
  revalidatePath('/area-lavoro/cantieri')
  return { success: true }
}

export async function markVisto(mediaId: number) {
  const db = await getConnection()
  try { await db.execute('ALTER TABLE cantieri_media ADD COLUMN visto TINYINT(1) NOT NULL DEFAULT 0') } catch { /* esiste già */ }
  await db.execute('UPDATE cantieri_media SET visto = 1 WHERE id = ?', [mediaId])
  await db.end()
}
