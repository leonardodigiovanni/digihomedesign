...
export default function OrdineClient({ ordine, articoli }: { ordine: OrdineInfo; articoli: OrdineArticolo[] }) {
  ...
  usePageTitleOverride(`/area-lavoro/ordini-ricevuti/${ordine.id}`, ordine.numero ? `Ordine ${ordine.numero}` : null)
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div>
        <h2 className="effetto-3d" style={{ fontSize: 24, fontWeight: 700, margin: 0 }}>
          Ordine {ordine.numero || `#${ordine.id}`}<ShortcutStar href={`/area-lavoro/ordini-ricevuti/${ordine.id}`} small />
        </h2>
