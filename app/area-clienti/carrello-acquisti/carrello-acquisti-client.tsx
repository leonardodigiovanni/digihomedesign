...
export default function CarrelloAcquistiClient({
  ...
  useEffect(() => {
    if (hasOrfani) pulisciCarrelloAcquistiOrfani().then(() => router.refresh())
                  ...
                  {filtrate.length === 0 ? (
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(96px, 1fr))', gap: 8 }}>
                      {filtrate.map(c => {
                        ...
                        return (
                          <div key={c.id} onClick={() => setLacunaSelected(c.id)}
                            style={{ border: isSel ? '2px solid #266626' : '1px solid #ddd', borderRadius: 6, overflow: 'hidden', cursor: 'pointer', userSelect: 'none', background: isSel ? '#e8f4e8' : '#fff', display: 'flex', flexDirection: 'column' }}>
                            ...
                            <div style={{ padding: '6px 6px', textAlign: 'center' }}>
                              <span style={{ fontSize: 11, fontWeight: isSel ? 700 : 400, color: '#1a1a1a', lineHeight: 1.3 }}>
                                ...
                                {c.prezzo_vendita > 0 && <span style={{ color: '#555' }}> — €{fmt(Number(c.prezzo_vendita))}</span>}
                              </span>
                            </div>
                          </div>
                        )
