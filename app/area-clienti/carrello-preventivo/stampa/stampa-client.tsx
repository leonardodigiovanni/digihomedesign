...
export default function StampaProvvisorioClient({ data, tornaHref = '/area-clienti/carrello-preventivo' }: { data: StampaData; tornaHref?: string }) {
  ...
  async function handleCondividi() {
    ...
    try {
      ...
      const shareData = navigator.canShare?.({ files })
        ? { files, title: 'Preventivo provvisorio' }
        : { title: 'Preventivo provvisorio', url: window.location.href }
