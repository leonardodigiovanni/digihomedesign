...
export default async function Page() {
  ...
  const { prev, next } = getCategoryGroupNeighbors('metallurgia', 2222, disabledPages)
  const CERCA = 'Grondaie'
  ...
  return (
    <div className="fs-15" style={{ padding: '0 0 64px', color: '#444', lineHeight: 1.8 }}>
      <p className="fs-12" style={{ color: '#000', marginBottom: 8, textShadow: 'none' }}>
        <Link href="/metallurgia" style={{ color: '#888', textDecoration: 'underline' }}>Metallurgia</Link> / Grondaie<ShortcutStar />
      </p>
      <h1 className="effetto-3d fs-28" style={{ fontWeight: 700, marginBottom: 8 }}>Grondaie a Palermo</h1>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        <StickyBottomBarContent>
          ...
          <CtaPreventivo />
          <CtaCantiere />
          {next ? <Link href={next.href} className="btn-blue fs-12">{next.label} →</Link> : <NavDropdownTriggerButton dropdownId="cat-legno" label="Legno →" />}
        </StickyBottomBarContent>
      </div>
