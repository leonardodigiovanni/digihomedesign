...
export default async function Page() {
  ...
  const { prev, next } = getAntintrusioneNeighbors(214, disabledPages)
    ...
    <div className="fs-15" style={{ padding: '0 0 64px', color: '#444', lineHeight: 1.8 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        <StickyBottomBarContent>
          ...
          <CtaPreventivo />
          <CtaCantiere />
          {next ? <Link href={next.href} className="btn-blue fs-12">{next.label} →</Link> : <NavDropdownTriggerButton dropdownId="carpenteria" label="Carpenteria d'Arredo →" />}
        </StickyBottomBarContent>
      </div>
